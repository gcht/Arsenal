# 编程
- alt + enter  自动import

    | 功能        | 快捷键    |  说明  |
    | ----------------   | -----------------:        | :-------------------: |
    | 自动import          | atl　 + enter             |   重要                 |
    |  回到光标上一处       |   command + alt + 左箭头  |    重要                |
    |   转类定义           |   command + B            |                         |
    |    转接口实现        |    command+alt+B          |                       |

# 调试
- ctrl + R   运行
- ctrl + D   调试运行
- command + F2  停止运行

# 查看
- 查找类 ```command + O```  
- 回到光标上一处    ```alt + command + <--```
- 转到类的声明（定义）  ```command + B```
- 转接口实现    
```command+alt+B```  
```command+alt+鼠标左键```


# 查找
- 当前页搜索        ```command + F``` 
- 全局搜索         ```shift + command + F```  



# 查找
- 格式化代码: Command + Option + L






# z注册吗
去掉==
```
MNQ043JMTU---

```

# windows 10 破解
- 参考 https://www.cnblogs.com/leton/p/12037696.html



# 导入工程
1. 导入工程    
New > Project From Existing Sources
2. maven环境配置
3. rebuild （有两个口）
- 鼠标选中工程邮件菜单
- IDEA菜单栏【Build】> 【Rebuild Project】

# maven环境配置
- IDEA菜单中【Preference】中，也可快捷键command+,（逗号）进入
- Build > Build Tools > Maven 在右侧的【User settings file】选择Override，填入【/Users/gct/.m2/ucf-mdf-parent/settings.xml】
> 其中settings.xml来自工程代码里的settings.xml
> 【推荐】在.m2下以工程名作为文件夹做隔离，每个工程文件夹下有自己的settings.xml和repository
